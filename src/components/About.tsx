...
const About = () => {
  ...
  const EducationCard = ({ 
    ...
    >
      <motion.div
        whileHover={{ y: -5 }}
        className="flex gap-4 bg-white/5 backdrop-blur-sm p-6 rounded-xl shadow-lg hover:shadow-purple-500/20 transition-all border border-white/10"
      >
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-2">
            <div>
              <h4 className="font-bold text-white/90 text-lg mb-1">{school}</h4>
              <p className="text-purple-400 font-medium">{degree}</p>
            </div>
            {year && (
              <span className="px-3 py-1 bg-gradient-to-r from-purple-500/30 to-pink-500/30 text-white rounded-full text-sm font-medium border border-purple-500/30">
                {year}
              </span>
            )}
          </div>
  ...
  const HighlightCard = ({ icon: Icon, title, description, color }: {
    ...
    <motion.div
      ...
      viewport={{ once: true }}
      whileHover={{ y: -5 }}
      className={`p-6 rounded-xl ${color} shadow-lg hover:shadow-purple-500/25 transition-all backdrop-blur-sm border border-white/10`}
    >
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl font-bold text-white/90 mb-4">About Me</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <motion.div 
            ...
            className="lg:pl-8"
          >
            <h3 className="text-2xl font-bold text-white/90 mb-8 text-center lg:text-left">Education Journey</h3>
